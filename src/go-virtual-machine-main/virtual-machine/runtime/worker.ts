...
export function setLocalThread(t: Thread) {
  local_thread = t
}

export function clearLocalThread() {
  local_thread = undefined as unknown as Thread
}
...
const object_to_instr = (object: Instruction): Instruction => {
  ...
  switch (tag) {
    ...
    case 'EXIT_BLOCK': {
      return new ExitBlockInstruction()
    }
    case 'BUILTIN_LEN': {
      return new BuiltinLenInstruction()
    }
    case 'BUILTIN_CAP': {
      return new BuiltinCapInstruction()
    }
    case 'FORK': {
    ...
    case 'LDCH': {
      return new LoadChannelInstruction()
    }
    case 'LDCR': {
    ...
    case 'TRY_CHAN_REQ': {
      return new TryChannelReqInstruction()
    }
    case 'SELECT CASES': {
    ...
    case 'LDC': {
      ...
      return new LoadConstantInstruction(
        obj_instr.val,
        object_to_type(obj_instr.data_type),
      )
    }
    case 'LDD': {
    ...
    case 'LDS': {
      return new LoadSliceInstruction()
    }
    case 'LD': {
    ...
    case 'LDP': {
      return new LoadPackageInstruction()
    }
    case 'LDSF': {
    ...
    case 'SLICEOP': {
      return new SliceOperationInstruction()
    }
    case 'SELECTOP': {
      return new SelectorOperationInstruction()
    }
    case 'STORE': {
  ...
  if (tag.startsWith('STORE ARRAY ELEMENT ')) {
    ...
    return new StoreStructFieldInstruction(
      obj_instr.index,
      obj_instr.order,
      obj_instr.hasKey,
      obj_instr.toPop,
  ...
  switch (tag) {
    ...
    case 'CHANNEL': {
      ...
      return new ChannelType(
        object_to_type(obj_type.element),
        obj_type.readable,
        obj_type.writable,
      )
    }
    ...
    case 'FUNCTION': {
      ...
      return new FunctionType(
        params,
        object_to_type(obj_type.results) as ReturnType,
        obj_type.variadic,
      )
    }
    case 'INT64': {
