...
export class Scheduler {
      ...
      switch (type) {
        case MessageType.READY: {
          ...
          for (
            ...
            const active_runqueue = this.heap.get_value(
              this.runqueues[active_id],
            ) as RunQueueNode
            ...
            const idle_runqueue = this.heap.get_value(
              this.runqueues[idle_id],
            ) as RunQueueNode
        ...
        case MessageType.BLOCK: {
          ...
          for (let i = 0; i < obj_addrs.length; i++) {
            ...
            const obj = this.heap.get_value(obj_addr) as
              | MutexNode
              | ChannelNode
              | WaitGroupNode
            ...
            if (generation !== obj.get_generation()) {
              ...
              const from_runqueue = this.heap.get_value(
                this.runqueues[thread_id],
              ) as RunQueueNode
        ...
        case MessageType.UNBLOCK: {
          for (let i = 0; i < event.data.obj_addrs.length; i++) {
            ...
            const unblocked_context = this.blocking_waitlists
              .get(obj_addr)
              ?.shift()
            ...
            const ctx = this.heap.get_value(
              unblocked_context_addr,
            ) as ContextNode
            ctx.set_blocked(false)
        ...
        case MessageType.NEW_GOROUTINE: {
          ...
          const old_runqueue = this.heap.get_value(
            this.runqueues[curr_id],
          ) as RunQueueNode
  ...
  assign_goroutine_to_runqueue(ctx_addr: number) {
    ...
    if (has_idle) {
      ...
      const runqueue = this.heap.get_value(
        this.runqueues[idle_id],
      ) as RunQueueNode
  ...
  log_gc_stats() {
    ...
    const mutator_time =
      this.gc_profiler.program_time - this.gc_profiler.total_pause_time
    const throughput_ratio = mutator_time / this.gc_profiler.program_time
    const alloc_rate =
      this.gc_profiler.total_alloc / this.gc_profiler.program_time
    const free_rate =
      this.gc_profiler.total_freed / this.gc_profiler.program_time
    ...
    console.log(
      'Avg GC Time: %f',
      this.gc_profiler.total_gc_time / this.gc_profiler.num_gc,
    )
  ...
  get_contexts_addr(): number[] {
    ...
    return [...running_contexts_addr.flat(), ...this.blocked_contexts]
  }
  ...
  static get_contexts(
    ...
    return [...running_contexts_addr.flat(), ...blocked_contexts].map(
      (x) => new ContextNode(heap, x),
    )
  }
}
